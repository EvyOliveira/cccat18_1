...
app.post("/signup", async function (req, res) {
	...
	let result;
	const [accountId] = await connection.query("select * from ccca.account where email = $1", [input.email]);

	if (accountId) return res.status(422).json({ message: -4 });
	...
	if (typeof result === "number") { res.status(422).json({ message: result }); } else { res.json(result); }
});
